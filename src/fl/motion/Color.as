package fl.motion
{
   ...
   public class Color extends ColorTransform
   {
      ...
      {
         return !!this.redOffset?Number(Number(1 - this.redMultiplier)):Number(Number(this.redMultiplier - 1));
      }
      ...
      private function parseXML(param1:XML = null) : Color
      {
         ...
         var _loc5_:XML = param1.elements()[0];
         if(!_loc5_)
         {
            return this;
         }
         for each(_loc2_ in _loc5_.attributes())
         {
            _loc3_ = _loc2_.localName();
            if(_loc3_ == "tintColor")
            {
               _loc4_ = Number(_loc2_.toString()) as uint;
               this.tintColor = _loc4_;
            }
            else
            {
               this[_loc3_] = Number(_loc2_.toString());
            }
         }
         return this;
      }
