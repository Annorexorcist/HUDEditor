...
package Shared.AS3
{
    ...
    public class ListFilterer extends EventDispatcher 
    {
        ...
        public function FindArrayIndexOfFilteredPosition(_arg_1:int):int
        {
            var _local_2:Number = this.ClampIndex(0);
            while (((_arg_1 > 0) && (!(_local_2 == int.MAX_VALUE))))
            {
                _local_2 = this.GetNextFilterMatch(_local_2);
                _arg_1--;
            };
            return (_local_2);
        }
        ...
        public function IsValidIndex(_arg_1:int):Boolean
        {
            return (((!(_arg_1 == int.MAX_VALUE)) && (!(this._filterArray == null))) && (this.EntryMatchesFilter(this._filterArray[_arg_1])));
        }
